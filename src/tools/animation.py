...
from tools.core import Node
from tools.transform import (lerp, quaternion_slerp, quaternion_matrix, translate,
                       scale, identity)
...
class Skinned:
    ...
    def draw(self, **uniforms):
        ...
        bone_matrices = world_transforms @ self.bone_offsets
        # Make sure the array contains MAX_BONES=128
        n = 128 - self.bone_offsets.shape[0]
        if n > 0:
            padding = np.array([np.eye(4, dtype=np.float32) for _ in range(n)])
            uniforms['bone_matrix'] = np.concatenate((bone_matrices, padding), axis=0)
        else:
            uniforms['bone_matrix'] = bone_matrices
